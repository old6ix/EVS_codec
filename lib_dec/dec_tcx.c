...
)
{
    ...
    if( bfi )
    {
        ...
        if( left_rect )
        {
            ...
            L_spec_con += st->tcx_cfg.tcx_coded_lines >> 2;
        }
    }
    else
    {
        ...
        if ( frame_cnt == 0 && (st->last_core_bs == ACELP_CORE) )
        {
            L_spec += st->tcx_cfg.tcx_coded_lines >> 2;
        }
    ...
    if( !bfi)
    {
        ...
        if(tcx_cfg->ctx_hm && ( (st->last_core_bs != ACELP_CORE) || (frame_cnt > 0) ) )
        {
        ...
        else  /* tcx_cfg->ctx_hm == 0 */
        {
            if( st->tcx_lpc_shaped_ari )  /* low rates: envelope based arithmetic coder */
            {
                ...
                tcx_arith_decode_envelope( x, L_frame, L_spec, st, coder_type, Aind, st->tcxltp_gain, *prm_target, prm_sqQ, st->last_core_bs != ACELP_CORE,
