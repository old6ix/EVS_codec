...
)
{
    ...
    if ( st->tcxonly )
    {
        st->core = get_next_indice(st, 1) + 1;
    ...
    if( !(st->core == ACELP_CORE && st->tcx_cfg.lfacNext<=0) && !st->use_partial_copy )
    {
        st->last_core_bs = get_next_indice(st, 1);
        ...
        */
        if( (!((st->last_total_brate == 0) && (st->clas_dec != VOICED_CLAS))) && (st->last_core_bs != st->last_core) )
        {
            st->last_core = st->last_core_bs;
        } 
    ...
    if( !st->use_partial_copy )
    {
        ...
        else if ((st->core == TCX_10_CORE) && (st->tcx_cfg.tcx_curr_overlap_mode == ALDO_WINDOW))
        {
            st->tcx_cfg.tcx_last_overlap_mode = FULL_OVERLAP;
        }
        else if ((st->core != TCX_10_CORE) && (st->tcx_cfg.tcx_curr_overlap_mode == FULL_OVERLAP))
        {
        ...
        if( st->core != ACELP_CORE )
        {
            ...
            if ((st->core == TCX_20_CORE) && (overlap_code == 0) && (st->last_core != ACELP_CORE) && (st->last_core != AMR_WB_CORE))
            {
                st->tcx_cfg.tcx_curr_overlap_mode = ALDO_WINDOW;
            }
        }
    ...
    if( st->use_partial_copy == 0 )
    {
        ...
        else
        {
            ...
            {
                if(st->sr_core == 16000 && *coder_type == VOICED && st->core == ACELP_CORE)
                {
                    lsf_bctcvq_decprm(st, param_lpc);
                }
                else
                {
                    lsf_msvq_ma_decprm( st, param_lpc, st->core, *coder_type, st->acelp_cfg.midLpc, st->narrowBand, st->sr_core );
                }
            }
            else
            {
    ...
    if( st->core == ACELP_CORE && st->use_partial_copy == 0 )
    {
        /* Target Bits */
    ...
    if( st->core == TCX_20_CORE && st->use_partial_copy == 0 )
    {
        flag_ctx_hm = 0;
        ...
        {
            SetTnsConfig(&st->tcx_cfg, 1, st->last_core_bs == ACELP_CORE);
        ...
        if( st->tcx_lpc_shaped_ari && st->last_core_bs != ACELP_CORE)
        {
        ...
        /*Context HM flag*/
        if ( st->tcx_cfg.ctx_hm && (st->last_core_bs != ACELP_CORE) )
        {
            prm[j] = get_next_indice(st, 1);
            nbits_tcx--;
        ...
        if (st->igf)
        {
            ...
            IGFDecReadLevel( &st->hIGFDec, st, (st->last_core_bs == ACELP_CORE)?IGF_GRID_LB_TRAN:IGF_GRID_LB_NORM, 1 );

            IGFDecReadData( &st->hIGFDec, st, (st->last_core_bs == ACELP_CORE)?IGF_GRID_LB_TRAN:IGF_GRID_LB_NORM, 1 );
    ...
    if( st->core == TCX_10_CORE )
    {
        ...
        for( k = 0; k < 2; k++ )
        {
            ...
            if( k==0 && st->last_core_bs == ACELP_CORE )
            {
                /* ACE->TCX transition */
                lgFB += lgFB >> 1;
            }
            ...
            if( st->tcx_cfg.fIsTNSAllowed )
            {
                if((st->last_core_bs == ACELP_CORE) && (k == 0))
                {
                    st->BER_detect = 1;
                    st->last_core = TCX_20_CORE;
                    st->last_core_bs = TCX_20_CORE;
            ...
            /*Context HM flag*/
            if( st->tcx_cfg.ctx_hm && !(st->last_core_bs == ACELP_CORE && k == 0) )
            {
                prm[j] = get_next_indice(st, 1);
                nbits_tcx--;
