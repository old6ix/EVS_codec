...
)
{
    ...
    {
        rtpdumpDepackerError = EVS_RTPDUMP_DEPACKER_open(&rtpdumpDepacker, f_stream, st->sdp_hf_only != 0);
    ...
    {
        assert( st->bitstreamformat == VOIP_G192_RTP );
        /* open input file */
    ...
    if (st->bitstreamformat == VOIP_G192_RTP)
    {
    ...
    for( ; ; )
    {
        ...
        if( nextPacketRcvTime_ms == (unsigned int)(-1) && ( EVS_RX_IsEmpty(hRX) || nFramesFed < 2 ) )
            break;
