    ...
    else if ( left_rect==1 && last_core_bfi==ACELP_CORE ) /* Rectangular window (past-frame is ACELP) */
    {
        ...
        else
        {
            ...
            if(acelp_zir_len >= 2.0f * 64)
            {
                ...
                acelp_zir[acelp_zir_len-1] = 0.40f * acelp_zir[acelp_zir_len-1] +  0.35f * acelp_zir[acelp_zir_len-1] + 0.25f * acelp_zir[acelp_zir_len-2];
                acelp_zir[acelp_zir_len-2] = 0.40f * acelp_zir[acelp_zir_len-2] +  0.35f * acelp_zir[acelp_zir_len-1] + 0.25f * acelp_zir[acelp_zir_len-1];
...
)
{
    ...
    /*Get amplitude*/
    {
        ...
        {
            mdct_gains[i] = 1.0f/max(EPSILON, sqrt(RealData[i] * RealData[i] + ImagData[i] * ImagData[i]));
        }
    }
