...
)
{
    ...
    *vad_flag = wb_vad( st, fr_bands, localVAD, &noisy_speech_HO, &clean_speech_HO, &NB_speech_HO, &snr_sum_he, &localVAD_HE_SAD, &(st->flag_noisy_speech_snr) );
    ...
    if (st->Pos_relE_cnt < 20)  /* Ensure the level is high enough and cldfb decision is reliable */
    {
    ...
    vad_param_updt( st, pitch, voicing, old_pitch1, corr_shift, A );
