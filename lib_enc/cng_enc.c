...
)
{
    ...
    {
        if ( st->last_core_brate > SID_2k40 && (st->last_core == HQ_CORE || burst_ho_cnt > 0) && st->lp_sp_enr < 6.0f && (sp_enr - st->lp_sp_enr) > 4.0f && sp_enr > 6.0f )
        {
            st->lp_sp_enr = sp_enr;
            force_cn_step = 1;
        }
