                                 ...
                                 )
{
    ...
    for (tile_idx = 0; tile_idx < hGrid->nTiles; tile_idx++)
    {
        ...
        for (sfb = hGrid->sfbWrap[tile_idx]; sfb < hGrid->sfbWrap[tile_idx + 1]; sfb++)
        {
            ...
            sfbEnergyTileR  = EPSILON;
            sfbEnergyTileC  = EPSILON;
            sfbEnergyC      = EPSILON;
            ...
            {
                sfbEnergyR  = EPSILON + sum2_f(pMDCTSpectrum + swb_offset[ sfb ], width) / width;
                gain        = (float)(sfbEnergyR);

            }

            gain = 0.5f + (float) ((2.885390081777927f * logf(gain) + 16.f));
